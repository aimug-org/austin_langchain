...
class TrelloClient:
    def __init__(self):
        """Initialize Trello client with settings from environment."""
        self.api_key = settings.TRELLO_API_KEY
        self.api_token = settings.TRELLO_TOKEN
        self.base_url = settings.TRELLO_API_BASE_URL
        self.client = httpx.Client(timeout=30)  # 30 second timeout
        # Optional default board ID from settings
        self.default_board_id = getattr(settings, 'TRELLO_BOARD_ID', None)
    ...
    def get_default_board_lists(self) -> list:
        """
        Fetch all lists for the default board.
        
        Returns:
            list: List of list objects with name and ID.
            
        Raises:
            TrelloError: If the request fails or no default board is configured.
        """
        if not self.default_board_id:
            raise TrelloError("No default board configured")
        return self.get_board_lists(self.default_board_id)
