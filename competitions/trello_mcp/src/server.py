...
@mcp.tool()
def get_default_board_lists() -> Dict[str, Union[PyList[BoardList], str]]:
    """
    Fetch all lists from the default Trello board.
    
    Returns:
        List of BoardList objects containing name and ID, or error message.
    """
    try:
        lists = trello_client.get_default_board_lists()
        return {"lists": lists}
    except TrelloError as e:
        return handle_trello_error(e)
...
@mcp.tool()
def create_list_in_default_board(params: BoardList) -> Dict[str, str]:
    """
    Create a new list in the default board.
    
    Args:
        params: BoardList object containing name and optional position
        
    Returns:
        Dict containing the ID of the created list or error message
    """
    try:
        list_id = trello_client.create_list(params.name, trello_client.default_board_id, params.position)
        return {"id": list_id}
    except TrelloError as e:
        return handle_trello_error(e)
